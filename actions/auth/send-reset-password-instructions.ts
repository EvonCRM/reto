...
export const sendResetPasswordInstructions = actionClient
  ...
  .action(async ({ parsedInput }) => {
    ...
    // Log reset password link to console for development/testing when email is not configured
    console.log('🔒 Password Reset Link for', maybeUser.email, ':', `${getBaseUrl()}${Routes.ResetPasswordRequest}/${passwordRequest.id}`);
